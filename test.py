    ...
    with torch.no_grad():
        ...
        for i, (data) in enumerate(test_pbar):
            ...
            for x in range(len(batch_strs_gold)):
                ...
                #Scripts for WER without seperating words
                # wer_1 = calculate_wer(batch_strs_hyps[x], batch_strs_gold[x])
                # total_wer_1 += wer_1
                # total_word_1 += len(batch_strs_gold[x].split(" "))
                ...
                print("Target Transcript",gold)
                print("Predicted Transcript",hyp)
                print("-"*50)
